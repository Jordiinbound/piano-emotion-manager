...
/**
 * Generate pianos list Excel
 */
export async function generatePianosExcel(pianos: any[]): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Pianos');
  ...
  pianos.forEach((piano) => {
    worksheet.addRow({
      ...
      lastServiceDate: piano.lastServiceDate ? new Date(piano.lastServiceDate).toLocaleDateString('es-ES') : '',
      createdAt: piano.createdAt ? new Date(piano.createdAt).toLocaleDateString('es-ES') : '',
    });
  });
  ...
  pianos.forEach((piano) => {
    typeCount[piano.type] = (typeCount[piano.type] || 0) + 1;
  });

  statsSheet.addRow({ metric: 'Total de pianos', value: pianos.length });
  Object.entries(typeCount).forEach(([type, count]) => {
    statsSheet.addRow({ metric: `Pianos tipo ${type}`, value: count });
  });
