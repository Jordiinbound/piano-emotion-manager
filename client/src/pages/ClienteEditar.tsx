...
export default function ClienteEditar() {
  ...
  const { data: client, isLoading } = trpc.clients.getClientById.useQuery({ id: clientId });
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    address: '',
    clientType: 'particular' as 'particular' | 'student' | 'professional' | 'music_school' | 'conservatory' | 'concert_hall',
    notes: '',
    region: '',
    city: '',
    postalCode: '',
  ...
  useEffect(() => {
    if (client) {
      setFormData({
        ...
        clientType: (client.clientType as any) || 'particular',
        notes: client.notes || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(t('clients.couldNotUpdateClient'));
    }
  };
  ...
  if (!client) {
    return <div className="container mx-auto py-6">{t('clients.clientNotFound')}</div>;
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 justify-end">
              ...
              <Button type="submit" disabled={updateMutation.isPending}>
                ...
                {updateMutation.isPending ? t('common.saving') : t('clients.updateClient')}
              </Button>
            </div>
          </form>
        </CardContent>
